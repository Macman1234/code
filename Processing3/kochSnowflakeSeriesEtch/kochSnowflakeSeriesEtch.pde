...
void setup() {
  size(960, 960);
  background(255);
  pdf = createGraphics(width, height, PDF, "kochSnowflakeSeriesEtch.pdf");
  pdf.beginDraw();
  pdf.rectMode(CORNERS);
  pdf.strokeWeight(0.072);
  //This magic box must match the box used by kochSnowflakeSeriesOutline.
...
void drawSnowflake(float sizeScalar) {
  ...
  for (int rotationCtr =  0; rotationCtr < rotations.length; ++rotationCtr) {
    ...
    if (rotationCtr >=  rotations.length * 4/12  && rotationCtr <=  rotations.length * 5/ 12 ) {
      pdf.vertex(x, y);
    } 
